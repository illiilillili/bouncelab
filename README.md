...
컨트롤 룰렛에서 컨트롤을 뽑으면 결과 카드의 두 버튼 아래에 **재미도 8칸**이 나옵니다.
처음에는 빈 칸이고, 하나를 누르면 그게 내 재미도가 됩니다. 그 칸을 고른 **사람 수가 칸 안에 숫자로** 보입니다.
한 사람 한 표이고, 다시 누르면 그 값으로 바뀝니다 (칸 안 숫자 = 그 칸을 고른 사람 수).
