...
- **바운스볼 뉴스** — 소식을 기사처럼 읽습니다. 목록은 `#/f/news`, 본문은 `#/f/news/<id>`. 목록은 오브젝트 평점 화면과 같은 카드에 부제와 분류 칩을 더한 모양이고, 기사는 분류 · 날짜 · 제목 · 부제 · 사진 · 리드 · 본문 순서로 나옵니다. **기사는 아직 하나도 없습니다** (`data/news.js` 가 빈 배열 — 쓸 내용을 받으면 그 파일에 넣습니다)
...
**기사는 아직 하나도 없습니다** — `window.BL.news` 가 빈 배열이라 목록 화면에는 `아직 기사가 없습니다` 안내만 나옵니다. 쓸 내용을 받아 항목을 넣으면 목록과 기사 화면이 바로 살아나고, 쓰는 방식(누가 어떻게 쓰는지)이 정해지면 이 파일만 바뀝니다.
