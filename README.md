...
- **바운스볼 뉴스** — 소식을 기사처럼 읽습니다. 목록은 `#/f/news`, 본문은 `#/f/news/<id>` (예: `#/f/news/roulette-difficulty`). 목록은 오브젝트 평점 화면과 같은 카드에 부제와 분류 칩을 더한 모양이고, 기사는 분류 · 날짜 · 제목 · 부제 · 사진 · 리드 · 본문 순서로 나옵니다. 지금은 `data/news.js` 에 적어 둔 예시 기사를 그대로 보여줍니다 (쓰는 방식은 아직 정하지 않았습니다)
...
**쓰는 방식(누가 어떻게 쓰는지)은 아직 정하지 않았습니다** — 지금은 파일에 직접 적는 방식이고, 방식이 정해지면 그 파일만 바뀝니다.
